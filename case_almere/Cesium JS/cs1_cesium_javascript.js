...
// Add the main sewage pipes from the municipality of Almere
      const objectTileset = scene.primitives.add(
        new Cesium.Cesium3DTileset({
          url: Cesium.IonResource.fromAssetId(1123828),
        })
      );

// Add the home connections from the municipality of Almere
      const object2Tileset = scene.primitives.add(
        new Cesium.Cesium3DTileset({
          url: Cesium.IonResource.fromAssetId(1123826),
        })
      );

// Add the parcels from the municipality of Almere
      const parcelTileset = scene.primitives.add(
        new Cesium.Cesium3DTileset({
          url: Cesium.IonResource.fromAssetId(1123830),
        })
      );
...
function update() {
  ...
  objectTileset.show = viewModel.objectTileset;
  object2Tileset.show = viewModel.object2Tileset;
...
// Toolbar buttons in order to fly to the overview and objects
Sandcastle.addDefaultToolbarButton("Overview", function () {  
  viewer.camera.flyTo({
  destination: Cesium.Cartesian3.fromDegrees(5.195122907651566 , 52.35496344912503, 500.0)
});
});

Sandcastle.addDefaultToolbarButton("Main sewage pipes", function () {
  viewer.flyTo(objectTileset);
});

Sandcastle.addDefaultToolbarButton("Home connections", function () {
  viewer.flyTo(object2Tileset);
});
