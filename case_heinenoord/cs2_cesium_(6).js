...
// Add the Heinenoordtunnel from 
// the municipalities of Heinenoord and Barendrecht
      const objectTileset = scene.primitives.add(
        new Cesium.Cesium3DTileset({
          url: Cesium.IonResource.fromAssetId(1118155),
        })
      );

// Add the parcels from the municipalities of Heinenoord and Barendrecht
      const parcelTileset = scene.primitives.add(
        new Cesium.Cesium3DTileset({
          url: Cesium.IonResource.fromAssetId(1118089),
        })
      );
...
Sandcastle.addDefaultToolbarButton("Overview", function () {  
  viewer.camera.flyTo({
  destination: Cesium.Cartesian3.fromDegrees(4.511464 , 51.832761, 5000.0)
});
});

Sandcastle.addDefaultToolbarButton("Heinenoordtunnel", function () {
